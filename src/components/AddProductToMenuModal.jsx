...
const AddProductToMenuModal = ({ isOpen, onClose, onSuccess }) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        if (result.errors) {
          ...
          toast.error(result.message || 'Ürün oluşturulamadı');
        }
      }
    } catch (err) {
      ...
      >
        ...
        <div className="p-6 border-b border-[#e7edf3] dark:border-slate-800 flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-[#4c739a] mt-1">Ürün bilgilerini girin ve menüye ekleyin</p>
          </div>
          <button
            onClick={handleClose}
            disabled={loading}
